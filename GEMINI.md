...
* **Unit Testing, CI & Pre-Commit Enforcement**: Comprehensive unit and integration tests across all test suites (`test/app.test.js`, `test/wotd-utils.test.js`, `test/generate-words.test.js`) are executed before every commit via `.githooks/pre-commit` and gated in CI via `.github/workflows/ci.yml`.
